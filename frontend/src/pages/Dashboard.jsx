...
import { 
  ...
  Globe, FileText, Ghost, Key, Trash2, X
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  const [isRefreshing, setIsRefreshing] = useState(false); // Visual indicator for polling
  ...
  // Modified to support Background Polling
  const fetchCerts = useCallback(async (isBackground = false) => {
    ...
    else setIsRefreshing(true); // Spin the icon, but don't block UI

    // Only reset expansion on full reloads, not background polls
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
        // Poll silently (isBackground = true)
        fetchCerts(true);
        fetchMetadata();
    }, POLL_INTERVAL);
  ...
  }, [fetchCerts, fetchMetadata, user?.has_api_key]);
  ...
  const renderSourceIcon = (type) => {
    if (type === 'NETWORK') return <Globe className="w-3.5 h-3.5 text-blue-500" />;
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        {!user?.has_api_key ? (
             <div className="flex flex-col items-center justify-center py-20 animate-in fade-in zoom-in duration-300">
                <div className="bg-white p-12 rounded-2xl shadow-sm border border-slate-200 max-w-2xl w-full text-center">
                    <div className="mx-auto bg-blue-50 w-20 h-20 rounded-full flex items-center justify-center mb-6">
                        <Key className="w-10 h-10 text-blue-600" />
                    </div>
                    <h2 className="text-3xl font-bold text-slate-900 mb-4">Welcome to CertMonitor</h2>
                    <p className="text-slate-500 text-lg mb-8 max-w-lg mx-auto">
                        You're almost there! To start monitoring your certificates, you need to generate your unique API Key to connect your agents.
                    </p>
                    <button onClick={handleGenerateKey} disabled={isGenerating} className="px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all flex items-center gap-3 mx-auto">
                    ...
                    <div className="flex items-center gap-2 border-t md:border-t-0 pt-4 md:pt-0 border-slate-100">
                        ...
                        {filters.presence === 'MISSING' && (
                            <button onClick={handlePruneMissing} className="flex items-center gap-2 px-3 py-2 bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 rounded-lg text-sm font-medium transition-colors" title="Delete all certificates marked as Missing">
                                ...
                                <span className="hidden lg:inline">Delete Missing</span>
                            </button>
                        )}

                        <div className="w-px h-8 bg-slate-200 mx-2 hidden md:block"></div>
                        
                        {/* Reset Filters Button (Visible only if active) */}
                        {hasActiveFilters && (
                            <button 
                                onClick={handleResetFilters}
                                className="flex items-center gap-2 px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg hover:bg-slate-100 hover:text-slate-700 transition-colors text-slate-500 text-sm font-medium" 
                        ...
                        <button onClick={handleManualRefresh} className="p-2 bg-slate-50 border border-slate-300 rounded-lg hover:bg-white hover:text-blue-600 transition-colors text-slate-500" title="Refresh Data">
                        ...
                        certs.map((cert) => (
                          <React.Fragment key={cert.id}>
                            <tr onClick={() => toggleRow(cert.id)} className={`hover:bg-slate-50 transition-colors cursor-pointer ${expandedRowId === cert.id ? 'bg-slate-50' : ''} ${cert.current_status === 'MISSING' ? 'opacity-70 grayscale-[0.5]' : ''}`}>
                              ...
                              <td className="px-6 py-4 font-medium text-slate-900"><div className="flex items-center gap-2">{cert.agent_hostname}{cert.current_status === 'MISSING' && (<div className="p-1 rounded bg-slate-100 border border-slate-200 text-slate-400" title="Missing from agent (Ghost)"><Ghost className="w-3.5 h-3.5" /></div>)}</div></td>
                              <td className="px-6 py-4 max-w-[180px] truncate" title={cert.subject.cn}>{cert.subject.cn}</td>
